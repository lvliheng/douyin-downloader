...
async def main_async(args):
    ...
    if args.latest_video_id:
        config.update(latest_video_id=args.latest_video_id)
...
def main():
    ...
    parser.add_argument("--last-video-id", help="指定最后已知视频 aweme_id（上一次下载的历史数据 id）")
    parser.add_argument("--latest-video-id", help="指定当前平台最新 aweme_id（新的数据，用于判断是否有新作品）")
